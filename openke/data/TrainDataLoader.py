...
class TrainDataLoader(object):
	...
	def __init__(self, 
		...
		# my attributes
		self.lib.WholeSampling.argtypes = [
			ctypes.c_void_p,
			ctypes.c_void_p,
			ctypes.c_void_p,
		]
	...
	def WholeSampling(self):
		self.lib.WholeSampling(
			self.batch_h_addr,
			self.batch_t_addr,
			self.batch_r_addr,
		)
		return {
			"batch_h": self.batch_h[:self.tripleTotal],
			"batch_t": self.batch_t[:self.tripleTotal],
			"batch_r": self.batch_r[:self.tripleTotal],
			# "batch_y": self.batch_y,
			"mode": "normal"
		}
	...
	def __iter__(self):
		if self.sampling_mode == 'WholeSampling':
			return TrainDataSampler(1, self.WholeSampling)
