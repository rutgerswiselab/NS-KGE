...
void sampling(
	...
	for (INT threads = 0; threads < workThreads; threads++) {
	    //printf("thread: %ld, workThreads: %ld\n", threads, workThreads);
...
void WholeSampling(
    ...
    for (INT batch = 0; batch < trainTotal; batch++) {
        batch_h[batch] = trainRel[batch].h;
        batch_t[batch] = trainRel[batch].t;
        batch_r[batch] = trainRel[batch].r;
    }
}
