...
void importTrainFiles() {
	...
	std::sort(trainRel, trainRel + trainTotal, Triple::cmp_rel2);
	...
	for (INT i = 1; i < trainTotal; i++) {
		...
		if (trainRel[i].r != trainRel[i - 1].r) {
			rigRel[trainRel[i - 1].r] = i - 1;
			lefRel[trainRel[i].r] = i;
		}
	}
	...
	lefRel[trainRel[0].r] = 0;
	rigRel[trainRel[trainTotal - 1].r] = trainTotal - 1;
